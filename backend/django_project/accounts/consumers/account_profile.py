...
class AccountProfileModelCollectionConsumer(AsyncJsonWebsocketConsumer):
    model_meta = AccountProfileModel._meta
    

    @property
    def nome_do_grupo(self):
        return f"accounts_collection_accountprofilemodel"
...
class AccountProfileModelConsumer(AsyncJsonWebsocketConsumer):
    object_pk =-1
    model_meta = AccountProfileModel._meta
    
    @property
    def nome_do_grupo(self):
        return f"AccountProfileModel_{self.object_pk}" 
    ...
    async def connect(self):
        ...
        if self.user.is_authenticated:
            ...
            if can_view:                
                ...
                await self.channel_layer.group_add(self.nome_do_grupo, self.channel_name)        
                await self.accept()
                app.send_task("stream_live_update_accountprofilemodel",[self.object_pk])
        else:
            await self.close()
